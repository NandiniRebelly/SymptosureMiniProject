...
#SEE THIS SI FOR CORRECT LOGIN,REGISTER EVEN THOUGH MONODB STOPPED WORKING
# import base64
# import json
# import os
# import socket
...
# SEE THIS VERSION IS FOR LAB TESTS OR MEDICAL TESTS RECOMMENDATIONS
import base64
import csv
import json
import os
...
def load_disease_tests() -> Dict[str, Dict[str, Any]]:
    mapping: Dict[str, Dict[str, Any]] = {}
    candidates = [Path("disease_tests.csv"), Path("artifacts") / "disease_tests.csv"]
    ...
    try:
        with open(csv_file, "r", encoding="utf-8") as f:
            ...
            for row in reader:
                disease = normalize_disease_key(row.get("Disease", ""))
                if not disease:
                    continue
                tests = [x.strip() for x in str(row.get("Recommended_Tests", "")).split(";") if x.strip()]
                mapping[disease] = {
                    "severity_trigger": str(row.get("Severity_Trigger", "High")).strip() or "High",
        ...
        print(f"⚠️ Failed to load disease tests: {e}")
    return mapping

def get_recommended_tests(disease_name: str, severity_en: str, emergency_flag: bool = False) -> List[str]:
    if not disease_name:
        return []
    config = disease_tests_map.get(normalize_disease_key(disease_name))
    if not config:
        return []
    if emergency_flag:
        return list(config.get("recommended_tests", []))
    trigger = str(config.get("severity_trigger", "High")).strip().lower()
    if trigger == "high" and severity_en == "High":
        return list(config.get("recommended_tests", []))
    return []

def localize_tests(tests: List[str], lang: str) -> List[str]:
    return translate_many_parallel(tests, lang)
...
def build_summary(disease_name_en: str, severity_en: str, lang: str, emergency_flag: bool) -> str:
    ...
    if emergency_flag:
        summary += f" 🚨 {t(lang, 'emergency')}"
    summary += build_tests_summary_text(disease_name_en, severity_en, lang, emergency_flag)
    return summary


def build_tests_summary_text(disease_name_en: str, severity_en: str, lang: str, emergency_flag: bool) -> str:
    tests_en = get_recommended_tests(disease_name_en, severity_en, emergency_flag)
    if not tests_en:
        return ""
    tests_local = localize_tests(tests_en, lang)
    if lang == "hi":
        return " अनुशंसित जाँचें: " + ", ".join(tests_local) + "."
    if lang == "te":
        return " సూచించిన పరీక్షలు: " + ", ".join(tests_local) + "."
    if lang == "ta":
        return " பரிந்துரைக்கப்பட்ட பரிசோதனைகள்: " + ", ".join(tests_local) + "."
    if lang == "kn":
        return " ಶಿಫಾರಸು ಮಾಡಿದ ಪರೀಕ್ಷೆಗಳು: " + ", ".join(tests_local) + "."
    if lang == "pa":
        return " ਸਿਫਾਰਸ਼ ਕੀਤੇ ਟੈਸਟ: " + ", ".join(tests_local) + "."
    return " Recommended tests: " + ", ".join(tests_local) + "."
...
def load_artifacts():
    global model_data, meta_data, disease_precautions, symptom_descriptions, disease_tests_map
    artifacts_dir = Path("artifacts")
    model_path = artifacts_dir / "model.joblib"
    meta_path = artifacts_dir / "meta.json"
    precautions_path = artifacts_dir / "disease_precaution_map.json"
    ...
    disease_tests_map = load_disease_tests()
    if disease_tests_map:
        print(f"✓ Loaded disease test map: {len(disease_tests_map)} diseases")
...
def detect_chat_intent(question_en: str) -> str:
    ...
    keywords = {
        ...
        "precautions": ["precaution", "precautions", "why", "care", "avoid", "what should i do", "what to do"],
        "summary": ["summary", "explain", "about disease", "tell me about", "what is this disease"],
        "tests": ["test", "tests", "lab test", "blood test", "medical test", "scan", "x-ray", "ecg", "mri", "ct"],
    }
...
def build_test_text(disease: str, severity: str, emergency_flag: bool = False) -> str:
    tests = get_recommended_tests(disease, severity, emergency_flag)
    if not tests:
        return "Routine lab or imaging tests are not usually recommended for this prediction unless symptoms become severe, persist, or a doctor advises testing."
    return f"For {disease}, recommended tests are: {', '.join(tests)}."
...
def generate_chat_answer_en(request: ChatRequest, question_en: str) -> str:
    ...
    if intent == "medicine":
        ...
        answer = build_test_text(disease, severity, emergency_flag)
    elif intent == "symptoms":
        answer = build_symptom_text(symptoms, symptom_desc)
    elif intent == "severity":
        answer = build_severity_text(severity, emergency_flag)
...
async def predict(request: PredictRequest):
    try:
        ...
        for item in selected:
            ...
            prob_percent = round(float(item.get("score", 0)) * 100, 1)
            recommended_tests_en = get_recommended_tests(disease, severity_en, emergency_flag)
            symptom_desc_en = {s: symptom_descriptions.get(s, f"{s.replace('_', ' ').title()} detected.") for s in matched}
            disease_predictions.append(DiseasePrediction(
                disease=disease,
                prob=prob_percent,
                ...
                matched_symptoms=matched,
                emergency=emergency_flag,
                recommended_tests=recommended_tests_en,
            ))
            ...
            if request.language != "en":
                predictions_translated.append(DiseasePrediction(
                    ...
                    matched_symptoms=localize_symptoms(matched, request.language),
                    emergency=emergency_flag,
                    recommended_tests=localize_tests(recommended_tests_en, request.language),
                ))
